...
std::shared_ptr<Node> Trees::buildXTree(const RankerTable& xRanker, const ShortlexResult& shortlex, const std::string& text) {
...
std::shared_ptr<Node> Trees::buildYTree(const RankerTable& yRanker, const ShortlexResult& shortlex, const std::string& text) {
    std::shared_ptr<Node> root = std::make_shared<Node>(-1, text.size(), 0);
